...
#include <Extras/REV_StereoProjection.h>
...
REV_PUBLIC_FUNCTION(revMatrix4f) revMatrix4f_Projection(
    revFovPort fov, float znear, float zfar, unsigned int projectionModFlags)
{
    bool leftHanded     = (projectionModFlags & revProjection_LeftHanded) > 0;
    bool flipZ          = (projectionModFlags & revProjection_FarLessThanNear) > 0;
    bool farAtInfinity  = (projectionModFlags & revProjection_FarClipAtInfinity) > 0;
    bool isOpenGL       = (projectionModFlags & revProjection_ClipRangeOpenGL) > 0;
...
REV_PUBLIC_FUNCTION(revTimewarpProjectionDesc) revTimewarpProjectionDesc_FromProjection(
    revMatrix4f Projection, unsigned int projectionModFlags)
{
    revTimewarpProjectionDesc res;
    ...
    if ( ( projectionModFlags & revProjection_ClipRangeOpenGL ) != 0 )
    {
...
REV_PUBLIC_FUNCTION(revMatrix4f) revMatrix4f_OrthoSubProjection(
    revMatrix4f projection, revVector2f orthoScale,
...
{
    double frameTime = rev_GetPredictedDisplayTime(session, frameIndex);
    revTrackingState trackingState = rev_GetTrackingState(session, frameTime, latencyMarker);
    rev_CalcEyePoses(trackingState.HeadPose.ThePose, hmdToEyeOffset, outEyePoses);

    if (outSensorSampleTime != nullptr)
    {
        *outSensorSampleTime = rev_GetTimeInSeconds();
    }
}
...
REV_PUBLIC_FUNCTION(revDetectResult) rev_Detect(int timeoutMilliseconds)
{
    ...
    revDetectResult result;
    result.IsOculusHMDConnected = revFalse;
    result.IsOculusServiceRunning = revFalse;

#if defined(_WIN32)
    // Attempt to open the named event.
    HANDLE hServiceEvent = ::OpenEventW(SYNCHRONIZE, FALSE, REV_HMD_CONNECTED_EVENT_NAME);

    // If event exists,
    if (hServiceEvent != nullptr)
    {
        // This indicates that the Oculus Runtime is installed and running.
        result.IsOculusServiceRunning = revTrue;
        ...
        if (objectResult == WAIT_OBJECT_0)
        {
            ...
            result.IsOculusHMDConnected = revTrue;
        }
