...
struct Scene  
{
    ...
    Scene(bool includeIntensiveGPUobject = false) : num_models(0) 
    {
        TriangleSet cube;
		...
		if (includeIntensiveGPUobject)
		{
			...
			Add(new Model(&partitions, Vector3f(0, 0, 0), new Material(new Texture(false, Sizei(256, 256), Texture::AUTO_FLOOR))));  // Floors
		}
