...
#ifndef OVR_CAPI_0_4_h
#define OVR_CAPI_0_4_h
...
{
	// Read-only flags.
	ovrHmdCap_Present = 0x0001,   ///  The HMD is plugged in and detected by the system. -- HMD被插入并被系统检测到。
	ovrHmdCap_Available = 0x0002,   ///  The HMD and its sensor is available for ownership use. -- HMD及其传感器可用于所有权使用。
									///  i.e. it is not already owned by another application. -- 即它尚未被另一个应用程序所有。
									ovrHmdCap_Captured = 0x0004,   ///  Set to 'true' if we captured ownership of this HMD. -- 如果我们捕获了此HMD的所有权，则设置为“true”。

																   // These flags are intended for use with the new driver display mode. -- 这些标志旨在用于新的驱动程序显示模式。
																   ovrHmdCap_ExtendDesktop = 0x0008,   /// (read only) Means the display driver is in compatibility mode. -- （只读）表示显示驱动程序处于兼容模式。

																									   // Modifiable flags (through ovrHmd_SetEnabledCaps). -- 可修改的标志（通过ovrHmd_SetEnabledCaps）。
																									   ovrHmdCap_NoMirrorToWindow = 0x2000,   /// Disables mirroring of HMD output to the window. This may improve 
																																			  /// rendering performance slightly (only if 'ExtendDesktop' is off). -- 禁用将HMD输出镜像到窗口。 这可能会略微提高渲染性能（仅当“ExtendDesktop”关闭时）。
																																			  ovrHmdCap_DisplayOff = 0x0040,   /// Turns off HMD screen and output (only if 'ExtendDesktop' is off). -- 关闭HMD屏幕并输出（仅当“ExtendDesktop”关闭时）。
																																			  ...
																																			  ovrHmdCap_DynamicPrediction = 0x0200,   ///  Adjust prediction dynamically based on internally measured latency.
																																													  ...
																																													  // These bits can be modified by ovrHmd_SetEnabledCaps. -- 这些位可以通过 ovrHmd_SetEnabledCaps 进行修改。
																																													  ovrHmdCap_Writable_Mask = 0x33F0,
																																													  /// These flags are currently passed into the service. May change without notice. -- 这些标志当前传递给服务。 可能会在没有通知时改变
...
typedef struct ovrHmdDesc_
{
	...
	short       FirmwareMajor;
	short       FirmwareMinor;
	/// External tracking camera frustum dimensions (if present).  外部跟踪相机截头尺寸（如果存在）。
...
typedef struct ovrFrameTiming_
{
	...
	/// 自上一帧以来所经过的时间 ThisFrameSeconds值（可用于运动缩放）。
	///	这将被限制在不超过0.1秒，以防止由于加载或初始化暂停后过度移动。
	float			DeltaSeconds;
	...
	/// Absolute point when IMU expects to be sampled for this frame. -- IMU预期在此帧采样的绝对点。
...
extern "C" {
	...
	/// 平台特定功能来指定其输出将会是的应用程序窗口显示在HMD上。 仅在 ovrHmdCap_ExtendDesktop 标志为false时使用。
	/// Windows：与此窗口相关的SwapChain将显示在HMD上。在窗口坐标中指定“destMirrorRect”以指示将从“sourceRenderTargetRect”镜像的渲染目标输出的区域。
	/// 空指针表示“全尺寸”。
	...
	/// 所有跟踪接口函数都是线程安全的，允许跟踪状态从不同的线程采样。
	/// 配置跟踪启动传感器采样，启用由 ovrTrackingCaps 描述的指定功能。
	/// - supportedTrackingCaps 指定请求的支持。 该功能将成功即使这些 caps 不可用（即传感器或照相机被拔掉）。 
	///	  如果这样的设备插入以后，支持会自动启用。 软件会检查 ovrTrackingState.StatusFlags 的实时状态。
	/// - requiredTrackingCaps 指定呼叫时所需的传感器功能。如果它们不可用，则该函数将失败。 如果仅指定 supportedTrackingCaps，则默认传递 0。
	/// - 为 supportedTrackingCaps 和 requiredTrackingCaps 传递 0 以禁用跟踪。
	...
	/// 计算在HMD内渲染给定眼睛的建议纹理大小与给定的FOV锥体。 较高的FOV通常需要较大的纹理来保持质量。
	/// - pixelsPerDisplayPixel 指定渲染目标像素数量的比率在畸变的中心显示像素。 1.0是默认值。 较低的值可以提高性能。
	...
	//-------------------------------------------------------------------------------------
	// *****  SDK Distortion Rendering Functions -- SDK畸变(失真)渲染函数
	...
	// 这些函数支持通过直接的SDK渲染失真访问底层渲染API，如D3D或GL。这是推荐的方法，因为它可以更好地支持未来	Oculus硬件，并支持一系列低级优化。
	...
	/// 配置渲染并填充计算的渲染参数。可以多次调用此功能来更改渲染设置。eyeRenderDescOut是一个指向两个ovrEyeRenderDesc结构数组的指针
	///	用于返回每只眼睛的完整渲染信息。
	/// - apiConfig 提供了D3D / OpenGL的特定参数。 通过null关闭渲染并释放所有资源。
	/// - distortionCaps 描述所需的失真设置。
	...
	/// or overriden here.
	/// 生成每个眼睛失真网格。
	/// 失真能力将取决于'distortionCaps'标志。 用户应根据其设置使用适当的着色器进行渲染。
	/// 失真网格数据将被分配并写入ovrDistortionMesh数据结构，该数据结构应该使用ovrHmd_DestroyDistortionMesh显式释放。
	/// 用户应该调用ovrHmd_GetRenderScaleAndOffset获取uvScale和Offset值进行渲染。
	/// 函数不应该失败，除非这是一个配置或内存错误，在这种情况下ovrDistortionMesh值将被设置为null。
	/// 这是SDK中唯一依赖于眼睛浮雕的功能，目前从配置文件导入，或者在此覆盖。
	...
	/// 计算更新后的“uvScaleOffsetOut”，以便在事件之后渲染目标大小或视口时使用失真。如果需要，这可以用来调整渲染大小。
	...
	/// 是否延迟测试处理，并返回“TRUE”，如果指定rgb颜色应该用来清除屏幕。
